...
class Klienti(Person):
    def __init__(self, name, surname, pk, numurs):
      print("Klienta dati saglabati")
      super().__init__(name, surname, pk, numurs)
      print("")

    def klienta_info(self):
        return [self.name, self.surname, self.pk, self.numurs]
...
file=open("Klienta_atskaite.txt")



class Logs:
    layout=[[sg.T('klients')],
        [sg.T('Vards'), sg.I(key='name')],
        [sg.T('uzvards'), sg.I(key='surname')],
        [sg.T('personas kods'), sg.I(key='pk')],
        [sg.T('numurs'), sg.I(key='numurs')],
        [sg.T(key='-OUT-')],
    ...
    while True:
        ...
        if event==sg.WIN_CLOSED or event == 'Exit':
          ...
          klients1=Klienti(values['name'], values['surname'], values['pk'], values['numurs'])
